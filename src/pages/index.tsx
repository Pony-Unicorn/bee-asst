...
export default function Home() {
  const [addItemBookmark, itemsBookmark] = useBookmarkStore((state) => [state.addItem, state.items], shallow);
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center justify-between p-24 ${inter.className}`}>
      ...
      <div>
        {Object.keys(itemsBookmark).map((v) => (
          <div key={v}>{v}</div>
        ))}
      </div>

      <div>
        {Object.values(itemsBookmark).map((v) => (
          <div key={v.i}>
            <div>{v.i}</div>
            <div>{v.n}</div>
            <div>{v.ct}</div>
